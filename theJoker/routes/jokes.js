...
router.get('/', function (req, res, next) {
  var joke = getItem(joy) + " ... " + getItem(laughs) + " ... " + askStarter + getItem(askComment)
...
router.get('/:words', function (req, res, next) {
  ...
  function callback(error, response, body) {
    if (error) {
      ...
      var maxTone = _.max(tones, function (tone) {
        return tone.score
      });
      ...
      if (tell) {
        joke += tellStarter + getItem(tellComment)
      } else {
        joke += askStarter + getItem(askComment)
      }
